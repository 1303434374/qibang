//调用示例
// apicloud.api('news').query({},(ret) => {
//     console.log(ret)     
// })
// apicloud.api('news').query({
//     filter: {
//         fields: ['title', 'image'],
//         where: {},
//         limit: 3
...
function api(param, callback) {
    var app_id = "A6061399626287";
    var app_key = "49B312FC-DC65-7081-0E7F-4D23E375B53D";
    var client = new Resource(app_id, app_key);
    // if ($api.getStorage('token')) {
    //     client.setHeaders("authorization", $api.getStorage('token'));
    // }
    if (callback) {
        client.batch(param, function (ret) {
            callback(ret.data);
        });
    } else {
        return client.Factory(param);
    }
}
...
function Resource(appId, appKey, baseurl) {
    ...
    this.defaultactions = {
        ...
        'checkvercode': {
            method: "POST",
            params: ["mobile", "vercode"]
        },
        'resetByMobile': {
            method: "POST",
            params: ["mobile", "vercode", "username", "password"]
        },
    };
    this.headers = {};
...
Resource.prototype.batch = function (requests, callback) {
    for (var i = 0, len = requests.length; i < len; i++) {
        ...
        request["path"] = "/mcm/api/" + request["path"];
        if (request["method"] && request["method"].toUpperCase() === "GET" && request["body"] && request["body"]["filter"]) {
            request["path"] = request["path"] + "?filter=" + JSON.stringify(request["body"]["filter"]);
            delete request["body"];
        }
    }
    var ajaxConfig = {
